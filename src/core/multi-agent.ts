...
export async function runParallelRedundant<Ctx>(
  ...
  const evaluationInput = [
    'You are given a user query and two agent responses.',
    'Determine which response better answers the query and return only that response.',
    `User query: ${query}`,
    `Response from ${a1}: ${outputToString(res1.outcome.output)}`,
    `Response from ${a2}: ${outputToString(res2.outcome.output)}`
  ].join('\n');
