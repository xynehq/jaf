...
export const tableChartTool = createFunctionTool({
  name: 'tableChart',
  description: 'Convert structured data (CSV/JSON) into markdown tables or chart specifications',
  execute: async (params) => {
    const { data, format, options } = params as {
      data: string;
      format: 'csv' | 'json';
  ...
  parameters: [
    {
      ...
      description: 'The input data as CSV or JSON string',
      required: true
    },
    {
      name: 'format',
      type: ToolParameterType.STRING,
      description: 'Input data format: "csv" or "json"',
      required: true
    },
    {
      name: 'options',
      type: ToolParameterType.OBJECT,
      description: 'Configuration options for table/chart generation',
      required: true
    }
  ]
});
